function [spOut, vOut] = LSOmodelAExIF(spEx, spIn, DT, Iext)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Active Exponential Integrate-and-fire Model of LSO
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% exponential spike-generating parameters
gSG = 26.4e-6; % 26.4*1e-6[mS] = 26.4[nS] for spike generation
VT = -47.3; % [mV] soft spike threshold 
KT = 1.8; % [mV] slope factor 
Vcut = -10.0;% [mV] cutoff voltage
...
for t = 1:Nsteps-1
 ...
 iSG = gSG * KT .* exp( (u-VT)/KT ); % exponential spike-generating current
 iEx = gEx(t) * (Eex-u) * 1e-6; % [uA] excitatory synaptic current 
 iIn = gIn(t) * (Ein-u) * 1e-6; % [uA] inhibitory synaptic current
 itot = iLL + iKL + iSG + iEx + iIn + jext(t); % total current 
 ...
 % (2) if cutoff voltage is reached, count a spike, hold the membrane potential 
 %     at the cutoff voltage, and set the refractory counter
 elseif(v>=Vcut) 
  spOut(t) = 1; v = Vcut; refCount = Nref; 
